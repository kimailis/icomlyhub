...
Format: JSON object { "articles": [{ headline, summary, celebName, buzzScore (10-95), trend ("up"|"down"|"stable"|"volatile"), category, publishedAt (ISO date string) }] }.`;

    try {
        console.log('[Test] Requesting content with search (Simple Service)...');
        const data = await geminiService.generateContent(prompt, { useSearch: true });
